...
export default function ArticlesPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    const res = await fetch("/api/articles", {
      ...
      body: JSON.stringify({
        title,
        featuredImage: featuredImage || "https://unsplash.com",
        content,
        youtubeLink,
        author,
    ...
    <div className="space-y-6 view-fade">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-800 pb-4">
        ...
        <div className="flex items-center space-x-3">
          ...
          >
            {showForm ? "✕ Dismiss" : "＋ Write Article"}
          </button>
        </div>
      </div>
      ...
      {showForm && (
        ...
        >
          <div className="space-y-3">
            ...
            <div>
              <label className="block text-slate-400 mb-1">
                Embedded YouTube Video Link (Optional)
              </label>
              <input
                type="url"
                value={youtubeLink}
          ...
          <div className="space-y-3 flex flex-col justify-between">
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Write the main story body content here..."
                ...
                <tr key={art.id} className="hover:bg-slate-800/10 transition">
                  <td className="py-3 pl-1 font-bold text-white max-w-xs sm:max-w-md">
                    <div className="flex items-center space-x-2">
                      <span
                        className={`w-1.5 h-12 rounded shrink-0 ${art.status === "Published" ? "bg-emerald-400" : "bg-amber-400"}`}
                      />
                  ...
                  <td className="py-3 font-mono text-[11px] text-slate-400">
                    ...
                    <div
                      className={`text-[9px] font-bold uppercase mt-0.5 ${art.status === "Published" ? "text-emerald-400" : "text-amber-400"}`}
                    >
                      {art.status}
                    </div>
                  </td>
                  <td className="py-3 text-right pr-1 space-x-2 whitespace-nowrap">
                    {art.youtubeLink && (
                      <a
                        href={art.youtubeLink}
                        target="_blank"
                        rel="noreferrer"
                        className="px-2 py-1 rounded bg-rose-500/10 text-rose-400 font-bold border border-rose-500/20 text-[10px]"
                      >
                        YouTube
                      </a>
                    )}
                    <button
                      ...
                      className="px-2 py-1 rounded bg-rose-500/10 hover:bg-rose-500 text-rose-400 hover:text-white border border-rose-500/20 text-[10px] font-bold transition cursor-pointer"
                    >
                      Delete
                    </button>
                  </td>
