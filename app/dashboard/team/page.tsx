    ...
    <div className="space-y-6 view-fade">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-800 pb-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            Manage everyone displayed on the Team page.
          </p>
        </div>
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <input
                ...
                placeholder="Morning Show DJ"
                className="w-full bg-slate-950 border border-slate-800 rounded px-3 py-2 text-white outline-none"
              />
            </div>
            <div>
              <label className="block text-slate-400 mb-1">
                Category Roster Group *
              </label>
              <select
                value={category}
          ...
          <div className="space-y-3 flex flex-col justify-between">
            <div>
              ...
              <input
                ...
                placeholder="A catchphrase summary"
                className="w-full bg-slate-950 border border-slate-800 rounded px-3 py-2 text-white outline-none"
              />
            </div>
            <div>
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Write background overview..."
            ...
            <div className="flex justify-end space-x-2">
              ...
              >
                {editingMember ? "Update Member" : "Commit Member"}
              </button>
            </div>
          </div>
        </form>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        {filteredList.map((m) => (
          <div
            key={m.id}
            className="bg-slate-900 border border-slate-800 rounded-xl p-5 flex flex-col sm:flex-row gap-4 items-start shadow-sm group hover:border-slate-700 transition"
          >
            <div className="relative w-20 h-20 shrink-0 self-center sm:self-start">
              <Image
                src={m.photo || "https://unsplash.com"}
                alt={m.name}
                fill
                className="object-cover rounded-lg bg-slate-950 border border-slate-800"
              />
            </div>
            <div className="flex-1 min-w-0 space-y-2 w-full">
              <div className="flex items-start justify-between gap-2">
                ...
                <span className="text-[9px] font-extrabold uppercase px-1.5 py-0.5 rounded bg-slate-950 text-slate-400 border border-slate-800 shrink-0">
                  {m.category}
                </span>
              </div>
              <p className="text-xs text-slate-300 italic font-medium">
                &quot;{m.tagline || "No tagline provided."}&quot;
              </p>
              <p className="text-[11px] text-slate-400 line-clamp-2 leading-relaxed">
                {m.biography || "No biography filled out yet."}
              </p>
              {m.socialLinks && (
                <div className="text-[10px] font-mono text-slate-500 truncate">
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="text-emerald-500/80 hover:underline"
                  >
                    {m.socialLinks}
                  </a>
                </div>
              ...
              <div className="flex justify-end space-x-3 pt-2 border-t border-slate-800/60 text-xs">
                <button
                  onClick={() => startEdit(m)}
                  className="text-slate-400 hover:text-emerald-400 font-bold transition"
                >
                  Edit
                </button>
