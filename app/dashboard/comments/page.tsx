...
export default function CommentsPage() {
  ...
  const [targetType, setTargetType] = useState<"Article" | "Episode">(
    "Episode",
  );
  ...
  const handleSubmitReply = (id: string, e: React.FormEvent) => {
    ...
    setTypedReply("");
    setToast(
      "💬 Administrator response successfully attached to comment thread!",
    );
    ...
    <div className="space-y-6 view-fade">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-800 pb-4">
        <div>
          <h1 className="text-xl font-bold text-white uppercase tracking-wider">
            Visitor Comments Deck
          </h1>
          <p className="text-xs text-slate-400 mt-0.5">
            Manage visitor engagement and moderate listener interaction
            channels.
          </p>
        </div>
        <div className="flex items-center space-x-3">
          ...
          >
            {showSimForm ? "✕ Dismiss Panel" : "＋ Simulate Visitor Submission"}
          </button>
        </div>
      </div>
      ...
      {showSimForm && (
        <form
          ...
          className="bg-slate-900 border border-slate-800 rounded-xl p-5 grid grid-cols-1 md:grid-cols-2 gap-4 text-xs form-slide border-l-4 border-l-emerald-500"
        >
          <div className="space-y-3">
            <div className="text-[10px] font-bold uppercase tracking-wider text-emerald-400">
              🌐 Public Visitor Action Simulator
            </div>
            <div>
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  setTargetTitle(
                    type === "Episode"
                      ? "Live Studio Session Mix"
                      : "Station Launches New Morning Grid Slot",
                  );
                }}
                className="w-full bg-slate-950 border border-slate-800 rounded px-3 py-2 text-white outline-none"
              >
            ...
            <div>
              <label className="block text-slate-400 mb-1">
                Select Active Show/Story Target *
              </label>
              <select
                value={targetTitle}
              ...
              >
                {targetType === "Episode" ? (
                  <>
                    <option value="Live Studio Session Mix">
                      Live Studio Session Mix (Radio Episode)
                    </option>
                    <option value="Freelancing Without Burnout">
                      Freelancing Without Burnout (Radio Episode)
                    </option>
                  </>
                ) : (
                  <>
                    <option value="Station Launches New Morning Grid Slot">
                      Station Launches New Morning Grid Slot (News Article)
                    </option>
                    <option value="Top 10 Indie Audio Tracks This Summer">
                      Top 10 Indie Audio Tracks This Summer (News Article)
                    </option>
                  </>
                )}
              </select>
          ...
          <div className="space-y-3 flex flex-col justify-end">
            <div>
              ...
              <textarea
                ...
                placeholder="Type what the visitor writes under the thread..."
      ...
      <div className="space-y-4">
        {filteredList.length === 0 ? (
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-8 text-center text-xs text-slate-500 font-mono tracking-wide animate-pulse">
            📡 No active visitor comments found. Click the simulation button
            above to submit an entry!
          </div>
        ) : (
          filteredList.map((com) => (
            <div
              key={com.id}
              className="bg-slate-900 border border-slate-800 rounded-xl p-5 shadow-sm space-y-4 hover:border-slate-800/80 transition"
            >
              ...
              <div className="flex justify-end items-center space-x-3 text-xs pt-1 border-t border-slate-800/40">
                {activeReplyId !== com.id ? (
                  <button
                    onClick={() => {
                      ...
                      setTypedReply(com.replyText || "");
                    }}
                    className="text-xs font-bold text-slate-400 hover:text-emerald-400 font-sans transition cursor-pointer"
                  >
                  ...
                  <button
                    ...
                    className="text-xs font-bold text-slate-500 hover:text-white font-sans transition cursor-pointer"
                  >
                    Cancel
                  </button>
                )}
                ...
                <button
                  ...
                  className="text-xs font-bold text-rose-500/80 hover:text-rose-400 font-sans transition cursor-pointer"
                >
              ...
              {activeReplyId === com.id && (
                <form
                  onSubmit={(e) => handleSubmitReply(com.id, e)}
                  className="pt-2 flex items-center space-x-2 form-slide"
                >
                  <input
                    type="text"
                    required
                    ...
                    placeholder="Type official response context..."
                    className="flex-1 bg-slate-950 border border-slate-800 rounded px-3 py-1.5 text-xs text-white outline-none focus:border-emerald-500 transition"
                  />
                  <button
                    type="submit"
                    className="px-4 py-1.5 bg-emerald-500 text-slate-950 font-bold rounded text-xs cursor-pointer"
                  >
                    Submit Response
                  </button>
                </form>
