  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (editingEpisode) {
      ...
      if (res.ok) {
        ...
        setList([newEp, ...list]);
        setToast(
          "🎞️ New broadcast episode attached to parent program channel!",
        );
      }
    }
    ...
    <div className="space-y-6 view-fade">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-800 pb-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            Manage all episodes published under the various CBM Radio programs.
          </p>
        </div>
      ...
      {showForm && (
        ...
        >
          ...
          <div className="space-y-3 flex flex-col justify-between">
            ...
            <div>
              <label className="block text-slate-400 mb-1">
                Audio Audio Download Link (MP3/WAV)
              </label>
              <input
                type="url"
                value={downloadLink}
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Brief summary story details..."
            ...
            <div className="flex justify-end space-x-2">
              ...
              >
                {editingEpisode ? "Update Log" : "Save Episode"}
              </button>
            </div>
          </div>
        </form>
      ...
      <div className="space-y-4">
        {filteredList.map((ep) => (
          <div
            key={ep.id}
            className="bg-slate-900 border border-slate-800 rounded-xl p-5 flex flex-col md:flex-row gap-5 shadow-sm group hover:border-slate-700 transition"
          >
            ...
            <div className="flex-1 min-w-0 flex flex-col justify-between space-y-3 w-full">
              <div className="space-y-1">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="text-[9px] font-extrabold uppercase px-1.5 py-0.5 rounded bg-slate-950 text-slate-300 border border-slate-800">
                    📻 {ep.programTitle}
                  </span>
                </div>
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 pt-2 border-t border-slate-800/60 text-xs">
                <div className="flex items-center space-x-4 font-mono text-[10px] text-slate-500">
                  <div>
                    📅 Aired:{" "}
                    <span className="text-slate-400">{ep.publishDate}</span>
                  </div>
                  {ep.downloadLink && (
                    <div>
                      💾{" "}
